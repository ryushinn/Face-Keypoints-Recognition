...
def pred():
    ...
    f = open("result/submission_change_kernel_size.csv")
    points = []
...
def eval():
    ...
    img_new, ratio_H, ratio_W = rescale(img, (200, 200))

    img_new = cv2.cvtColor(img_new, cv2.COLOR_BGR2GRAY)[:, :, None]
    print(img_new.shape)
    with torch.no_grad():
        outputs = net(transforms.ToTensor()(img_new).view(-1, 1, 200, 200))

    points = []
    for i in range(98):
        points.append((int(float(outputs[0][2 * i]/ratio_W)), int(float(outputs[0][2 * i + 1]/ratio_H))))
    for p in points:
        cv2.circle(img, p, 1, (0, 255, 0), 2)
